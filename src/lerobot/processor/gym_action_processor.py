...
class Torch2NumpyActionProcessorStep(ActionProcessorStep):
    ...
    当策略的输出（通常是 torch.Tensor）需要传递给期望
    NumPy 数组的环境或组件时，该步骤很有用。

    Attributes:
        squeeze_batch_dim: 如果为 True，则在数组第一维大小为 1 时将其移除。
                           这对于将大小为 (1, D) 的批量化动作
                           转换为大小为 (D,) 的单个动作很有用。
    """
...
class Numpy2TorchActionProcessorStep(ProcessorStep):
    ...
    def __call__(self, transition: EnvTransition) -> EnvTransition:
        """如果动作存在，则将 numpy 动作转换为 torch 张量，否则直接透传。"""
        ...
        if action is not None:
            ...
            torch_action = to_tensor(action, dtype=None)  # 保留原始 dtype
